    ...
    <section className="space-y-6">
      <Card className="bg-slate-900/30">
        <CardHeader>
          ...
          <CardDescription>
            Финальная точка пайплайна. Здесь соберём карточки и упакуем их в Anki
            deck.
          </CardDescription>
        </CardHeader>
          ...
          {EXPORT_PIPELINE.map((step) => (
            ...
            >
              <div className="flex flex-wrap items-center gap-3">
                ...
                <Badge
                  variant="outline"
                  className="border-dashed text-xs uppercase tracking-wide"
                >
                  {step.status}
                </Badge>
              </div>
        ...
        <CardContent className="border-t border-white/5 pt-6">
          ...
          <p className="mt-3 text-sm text-muted-foreground">
            Как только появится API для anki-apkg-export, кнопки включатся и
            загрузка начнёт формировать архив автоматически.
          </p>
        </CardContent>
      </Card>
    </section>
