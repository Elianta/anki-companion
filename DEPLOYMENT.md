...
- Publish directory: `dist` (required by Netlify even though functions-only)
- Redirects handled in `backend/netlify.toml` to route `/api/*` and `/health` to the `api` function.
- Env vars: `OPENAI_API_KEY` (required), `ALLOWED_ORIGINS` (optional, comma-separated).
Deploy this as a Netlify site to expose the API at `/.netlify/functions/api`.
